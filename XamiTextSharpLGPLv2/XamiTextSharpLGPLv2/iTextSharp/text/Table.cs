...
namespace iTextSharp.text
{
    ...
    public class Table : Rectangle, ILargeElement
    {
        ...
        */
        ...
        public void AddCell(Phrase content, object location)
        {
        ...
        public void AddCell(string content, object location)
        {
            ...
            public void InsertTable(Table aTable, System.Drawing.Point p)
        {
            ...
            int rowCount = (int)p.X + 1 - rows.Count;
            int i = 0;
            if (rowCount > 0)
            {   //create new rows ?
                for (; i < rowCount; i++)
            ...
            ((Row)rows[(int)p.X]).SetElement(aTable, (int)p.Y);
        ...
        */
        ...
        public int EndHeaders()
        {
            ...
            lastHeaderRow = (int)curPosition.X - 1;
        ...
        private bool IsValidLocation(Cell aCell, System.Drawing.Point aLocation)
        {
            ...
            {        // if false : new location is already at new, not-yet-created area so no check
                ...
                int difx = ((rows.Count - aLocation.X) > aCell.Rowspan) ? aCell.Rowspan : rows.Count - (int)aLocation.X;
                int dify = ((columns - aLocation.Y) > aCell.Colspan) ? aCell.Colspan : columns - (int)aLocation.Y;
        ...
        private void PlaceCell(ArrayList someRows, Cell aCell, System.Drawing.Point aPosition)
        {
            int i;
            Row row = null;
            int rowCount = (int)aPosition.X + aCell.Rowspan - someRows.Count;
